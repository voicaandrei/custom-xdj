...
class BetaTraceBuild(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_trace_decoder_preserves_stock_five_argument_contract(self):
        ...
        disassembly = subprocess.run(
            [objdump, "-d", str(self.folder / "browser-waveform-trace-beta.elf")],
    ...
    def test_trace_stage_calls_are_linked_into_getimage_wrapper(self):
        ...
        symbols = subprocess.run(
            [tools["nm"], "-n", str(self.folder / "browser-waveform-trace-beta.elf")],
            check=True, capture_output=True, text=True,
        ).stdout
        for symbol in (
    ...
    @unittest.skipUnless((FOLDER / "XDJ1KMK2.UPD").exists(), "Test 05 absent")
