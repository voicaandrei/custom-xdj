...
class Beta16Behavior(unittest.TestCase):
    ...
    def test_exact_rows_bounds_releases_errors_and_non_info_passthrough(self):
        self.compile_run(['tests/native_beta_info14_test.c'])
    ...
    def test_historical_beta15_sources_unchanged(self):
        if not (ROOT/'evidence/browser-waveform-beta-v144-test15.json').exists(): self.skipTest('Private historical manifest not supplied')
        manifest=json.loads((ROOT/'evidence/browser-waveform-beta-v144-test15.json').read_text())
