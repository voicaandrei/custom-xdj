...
static uint8_t payload[7228], original[7228];
...
static int query(void *ctx,uint32_t id,const char *atom,const char *extension,
                 uint32_t *length,uint8_t **blob,uint32_t *metadata) {
    assert(ctx==&connection && id==3902);
    assert(!strcmp(atom,"PWV4") && !strcmp(extension,"EXT"));
    assert(!*metadata && !*length && !*blob);
    *metadata=0x12345678u; /* deliberately not treated as payload length */
...
int main(int argc,char **argv) {
    xdj_wave_entry entry; uint8_t order[1];
    ...
    xdj_rgb_source source={query,release,&connection,current,&connection};
    uint16_t pixels[80*28]; FILE *file; int scenario;
    assert(argc==2);file=fopen(argv[1],"rb");assert(file);
    assert(fread(original,1,7228,file)==7228);assert(fgetc(file)==EOF);fclose(file);
    assert(xdj_make_track_key(&id,expected_key));
    for (scenario=0;scenario<7;++scenario) {
        xdj_adapter_result result;
        memcpy(payload,original,7228); queries=releases=checks=0;
        status=1;stale=null_blob=0;reply_bytes=7228;
        ...
        if(scenario==3)reply_bytes=7227;
        if(scenario==4)null_blob=1;
        if(scenario==5)payload[0]=255;
        result=xdj_rgb_request_prepare(&source,&id,&cache,&layout,
        ...
        if(scenario==0) {
            ...
            assert(xdj_adapter_draw(&cache,expected_key,XDJ_PREVIEW_RGB,&layout,
                pixels,80*28,80,0,0xffff,1)==XDJ_ADAPTER_HIT);
            assert(xdj_rgb_request_prepare(&source,&id,&cache,&layout,
