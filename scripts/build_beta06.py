#!/usr/bin/env python3
"""Build the v1.44 local-only traced waveform beta used as physical Test 06.
...
FOLDER = ROOT / "private/browser-waveform-beta-v144-test06"
CODE_BASE = 0x08000000
ALIGNMENT = 16
MARKER_TEXT = "XDJ BETA 06".encode("utf-16le")
PATCHES = {
...
def link_beta(folder: Path, application_length: int) -> dict:
    ...
    raw = folder / "browser-waveform-beta06.bin"
    map_file = folder / "browser-waveform-beta06.map"
    run([
...
def build(destination: Path = FOLDER) -> dict:
    ...
    report = {
        "kind": "test-06-browser-waveform-local-only-trace-beta-v1",
        ...
        "text_marker_offsets": list(TEXT_OFFSETS), "text_marker": "XDJ BETA 06",
        ...
        "hardware_evidence_before_test06": {
            ...
            "test05_trace_observed": "one white bar over a blue strip in BROWSE and INFO",
            "test05_decoder_hook_and_bounded_surface_writes_observed": True,
        ...
        "known_beta_limits": [
            ...
            "The feature is always enabled in Test 06; the optional toggle follows device validation.",
            "Recovery from a checksum-valid application hang remains unverified.",
        ],
    }
    payload = json.dumps(report, indent=2) + "\n"
    (destination / "manifest.json").write_text(payload)
    (ROOT / "evidence/browser-waveform-beta-v144-test06.json").write_text(payload)
    return report
