#!/usr/bin/env python3
"""Build the v1.44 browser-waveform beta used as physical Test 04.
...
FOLDER = ROOT / "private/browser-waveform-beta-v144-test04"
CODE_BASE = 0x08000000
ALIGNMENT = 16
MARKER_TEXT = "XDJ BETA 04".encode("utf-16le")
PATCHES = {
...
def link_beta(folder: Path, application_length: int) -> dict:
    ...
    elf = folder / "browser-waveform-beta.elf"
    raw = folder / "browser-waveform-beta.bin"
    map_file = folder / "browser-waveform-beta.map"
    run([
...
def build(destination: Path = FOLDER) -> dict:
    ...
    report = {
        "kind": "test-04-browser-waveform-beta-v1",
        ...
        "text_marker_offsets": list(TEXT_OFFSETS), "text_marker": "XDJ BETA 04",
        ...
        "test03_hardware_evidence": {
            "main_modified_application_observed_running": True,
            "marker_observed": "XDJ TEST 02",
            "pixel_0x001f_observed": "blue in list and INFO",
        ...
        "local_verification": {
            ...
            "python_tests_passed": 162,
            "failures": 0,
            "independent_full_container_audit": True,
        },
        "known_beta_limits": [
            "Rows without an artwork request retain stock appearance in this beta.",
            "The PWV4 query and decoder hooks are ABI-derived statically and need this first device test.",
            "Blue uses selector zero; this exact selector still needs device validation.",
            "The feature is always enabled in Test 04; the optional toggle follows device validation.",
            "Recovery from a checksum-valid application hang remains unverified.",
        ],
    }
    payload = json.dumps(report, indent=2) + "\n"
    (destination / "manifest.json").write_text(payload)
    (ROOT / "evidence/browser-waveform-beta-v144-test04.json").write_text(payload)
    return report
