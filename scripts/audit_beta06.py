#!/usr/bin/env python3
"""Independent final audit for the exact v1.44 Test 06 traced beta."""
...
from build_beta06 import FORCE_AFTER, FORCE_OFFSET, PATCHES, TEXT_OFFSETS
...
OUTPUT = ROOT / "evidence/browser-waveform-beta-v144-test06-final-audit.json"
...
def audit():
    ...
    marker = "XDJ BETA 06".encode("utf-16le")
    ...
    guard = getimage.index("cmp/eq\t#1,r0")
    accepted = getimage.index("bt\t", guard)
    reset = getimage.index("_xdj_beta_trace_reset", accepted)
    row_read = getimage.index("mov\t#80,r0")
    assert guard < accepted < reset < row_read
    for instruction in (
    ...
    report = {
        "kind": "test-06-final-independent-container-audit",
        "date": "2026-09-09",
        "candidate": "private/browser-waveform-beta-v144-test06/XDJ1KMK2.UPD",
        ...
        "preview_reset_after_local_context_guard": True,
        ...
        "tests": {"node_passed": 33, "python_passed": 174, "failed": 0},
