#!/usr/bin/env python3
"""Hash-locked Test 16 builder; no USB operations or historical report writes."""
import hashlib
...
FOLDER = ROOT / 'private/browser-waveform-beta-v144-test16'
MARKER_TEXT = 'XDJ FINAL16'.encode('utf-16le')
spec = importlib.util.spec_from_file_location('_beta16_primitives', ROOT / 'scripts/build_beta06.py')
...
core.SOURCES = tuple(ROOT / p for p in (
    ...
    'native/waveform_cell.c', 'native/pixel_channels.c', 'native/v144/beta15_hooks.S', 'native/beta_info_band14.c', 'native/v144/info_band_hooks14.S',
...
core.HASH_SOURCES = core.SOURCES + tuple(ROOT / p for p in (
    ...
    'scripts/build_beta16.py', 'scripts/build_beta06.py',
    'scripts/index_beta16_contract.py', 'scripts/index_beta15_contract.py', 'scripts/index_beta14_contract.py', 'scripts/index_beta13_contract.py'))
...
def build(destination=FOLDER):
    from index_beta16_contract import inspect
    contract=inspect()
    ...
    report = {
        'kind': 'final-16-restore-80px-preserve-15-fixes', 'numeric_version': '1.44',
        'stock_contract':contract,
        'text_marker': 'XDJ FINAL16', 'text_marker_offsets': list(core.TEXT_OFFSETS),
        ...
        'browse_width':{'without_info':80,'with_info':80,'source_columns':80,
                         'display_method':'original 80px surface and stock row copy; width hooks removed',
                         'title_and_note_shift':0,'right_edge_unchanged':True},
        ...
        'limits': ['Blue/RGB worked in owner Test 09; fast scrolling regressed. Test 16 fix is not yet hardware validated.',
                   ...
                   'INFO transfer validated in Test 14; comment suppression and wide-row layout need Test 16 validation.',
...
if __name__ == '__main__':
    report = build()
    (ROOT / 'evidence/browser-waveform-beta-v144-test16.json').write_text(json.dumps(report, indent=2) + '\n')
