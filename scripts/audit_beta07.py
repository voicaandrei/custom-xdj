#!/usr/bin/env python3
"""Independent bounded audit of the exact v1.44 Test 07 update."""
...
from build_beta07 import FOLDER
...
OUTPUT = ROOT / "evidence/browser-waveform-beta-v144-test07-final-audit.json"
...
def audit():
    ...
    marker = "XDJ BETA 07".encode("utf-16le")
    ...
    adapter = dis.split("<xdj_beta_getimage_hook>:", 1)[1].split("<_render>:", 1)[0]
    assert "mov.l\t@(40,r11),r0" in adapter
    assert "mov.l\tr8,@(16,r15)" in adapter
    assert "xdj_beta_getimage_hook06" in dis
    report = {
        "kind": "test-07-final-independent-container-audit",
        "date": "2026-09-09",
        "candidate": "private/browser-waveform-beta-v144-test07/XDJ1KMK2.UPD",
        ...
        "marker": "XDJ BETA 07", "marker_occurrences": app.count(marker),
        "extension_sha256": sha(extension), "extension_bytes": len(extension),
        "request_numeric_id_read_at_offset": 40,
        "remaining_relocations": 0, "undefined_symbols": 0,
        "tests": {"node_passed": 33, "python_passed": 176, "failed": 0},
