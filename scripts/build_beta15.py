#!/usr/bin/env python3
"""Hash-locked Test 15 builder; no USB operations or historical report writes."""
import hashlib
...
FOLDER = ROOT / 'private/browser-waveform-beta-v144-test15'
MARKER_TEXT = 'XDJ BETA 15'.encode('utf-16le')
spec = importlib.util.spec_from_file_location('_beta15_primitives', ROOT / 'scripts/build_beta06.py')
...
core.SOURCES = tuple(ROOT / p for p in (
    ...
    'native/waveform_cell.c', 'native/pixel_channels.c', 'native/v144/beta15_hooks.S', 'native/beta_info_band15.c', 'native/v144/info_band_hooks14.S',
    'native/beta_browse_observed.c', 'native/beta_ui15.c', 'native/v144/ui15_hooks.S', 'native/beta_preview_scope15.c', 'native/beta_key_note.c', 'native/v144/key_note_hooks.S'))
core.HASH_SOURCES = core.SOURCES + tuple(ROOT / p for p in (
    ...
    'scripts/build_beta15.py', 'scripts/build_beta06.py',
    'scripts/index_beta15_contract.py', 'scripts/index_beta14_contract.py', 'scripts/index_beta13_contract.py'))
...
core.PATCHES.update({
    0x152e8f4:(0x0952cbac,'_xdj_beta_artwork_draw'),
    0x152c524:(0x09522202,'_xdj_beta_row_left'),
    0x152c2b8:(0x09522202,'_xdj_beta_row_left'),
    0x12c0eec:(0x092bd968,'xdj_beta_serialize_note'),
    0x12c1188:(0x092bd968,'xdj_beta_serialize_note'),
    0x152c24c:(0x2fd62fc6,'_xdj_beta_title'),
    0x152c024:(0x2fe62fd6,'_xdj_beta_note'),
})
for site in (0x152ae28,0x152b3a0,0x152c2ac,0x152c520,0x152c860,
             0x152f35c,0x152f5ec,0x152f6b8,0x152f880,0x1537c00):
    core.PATCHES[site]=(0x095242c8,'_xdj_beta_icon_resource')
ENTRY_PATCHES={0x152c244:'862f962fa62fb62fc62fd62f',
               0x152c01c:'962fa62fb62fc62fd62fe62f'}
...
def patch_application(reference, linked):
    ...
    for offset,expected in ENTRY_PATCHES.items():
        if reference[offset:offset+12].hex()!=expected:
            raise ValueError('UI entry anchor changed')
        application[offset:offset+8]=bytes.fromhex('01d12b4109000900')
    return bytes(application)
core.patch_application = patch_application


def build(destination=FOLDER):
    from index_beta15_contract import inspect
    contract=inspect()
    ...
    report = {
        'kind': 'test-15-ui-width-key-footer-and-preview-drain', 'numeric_version': '1.44',
        'stock_contract':contract,
        'text_marker': 'XDJ BETA 15', 'text_marker_offsets': list(core.TEXT_OFFSETS),
        ...
        'info_band': {'width': 288, 'height': 28, 'x': 500, 'y': 284,
                      ...
                      'position_status': 'position and undistorted transfer owner-confirmed in Test 14',
        ...
        'browse_width':{'without_info':160,'with_info':80,'source_columns':80,
                         'display_method':'duplicate each cached column horizontally; no extra DB work',
                         'title_and_note_shift':80,'right_edge_unchanged':True},
        'info_comment':{'hidden_group':1,'hidden_row':6,'controls':[12,13]},
        'preview_drain':{'mask_cancel_enable_during_preview':True,'restore_original_byte':True,
                         'cancel_byte_12_never_cleared':True,'extra_preview_requests':0},
        'ui_entry_patches':{hex(k):v for k,v in ENTRY_PATCHES.items()},
        'hardware_tested': False, 'copied_to_usb': False,
        'limits': ['Blue/RGB worked in owner Test 09; fast scrolling regressed. Test 15 fix is not yet hardware validated.',
                   ...
                   'INFO transfer validated in Test 14; comment suppression and wide-row layout need Test 15 validation.',
...
if __name__ == '__main__':
    report = build()
    (ROOT / 'evidence/browser-waveform-beta-v144-test15.json').write_text(json.dumps(report, indent=2) + '\n')
