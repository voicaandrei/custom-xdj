#!/usr/bin/env python3
"""Independent bounded audit of the exact v1.44 Test 16 update."""
...
from build_beta16 import FOLDER, PATCHES, COPY_OFFSET, COPY_BEFORE, KEY_ENTRY_OFFSET, KEY_ENTRY_BEFORE, ENTRY_PATCHES
...
from index_beta16_contract import inspect as inspect_beta16
...
OUTPUT = ROOT / "evidence/browser-waveform-beta-v144-test16-final-audit.json"
...
def audit():
    ...
    marker = "XDJ FINAL16".encode("utf-16le")
    ...
    assert inline.hex() == '09006a0189216a41fd54f855fb522c3552653fe61846007604d10b4109006a0189216a4106a00900090070655a09000000000000'
    expected[COPY_OFFSET:COPY_OFFSET+len(inline)] = inline
    assert stock_app[KEY_ENTRY_OFFSET:KEY_ENTRY_OFFSET+12] == KEY_ENTRY_BEFORE, ENTRY_PATCHES
    ...
    for offset, before in ENTRY_PATCHES.items():
        assert stock_app[offset:offset+12].hex()==before
        expected[offset:offset+8]=bytes.fromhex('01d12b4109000900')
    ...
    for off in (0x152e8f4,0x152c524,0x152c2b8):
        assert off not in PATCHES
        assert app[off:off+4]==stock_app[off:off+4]
    assert manifest['browse_width']['without_info']==80
    assert manifest['browse_width']['with_info']==80
    assert manifest['browse_width']['title_and_note_shift']==0
    assert 'native/beta_info_band14.c' in manifest['source_sha256']
    assert 'native/beta_info_band15.c' not in manifest['source_sha256']
    assert bytes(expected) == app
    assert app.count(marker) == len(TEXT_OFFSETS)
    assert manifest['info_band']['surface_width']==288
    assert manifest['info_band']['width']==288
    assert manifest['stock_contract']==contract
    ...
    report = {
        "kind": "test-16-final-independent-container-audit",
        "date": "2026-09-09",
        "candidate": "private/browser-waveform-beta-v144-test16/XDJ1KMK2.UPD",
        ...
        "marker": "XDJ FINAL16", "marker_occurrences": app.count(marker),
        ...
        "cancellation_handling": "stock context+12 preserved; preview scope temporarily disables +13 until reply release, then restores original value",
        ...
        "tests": json.loads((ROOT/"evidence/test16-validation.json").read_text()),
        "key_note_stock_evidence": key_evidence,
        "info_band": manifest["info_band"],
        "browse_settle": manifest["browse_settle"],
        "stock_contract": contract,
        "browse_width":manifest["browse_width"],
        "info_comment":manifest["info_comment"],
        "preview_drain":manifest["preview_drain"],
