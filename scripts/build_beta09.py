#!/usr/bin/env python3
"""Hash-locked Test 09 builder; no USB operations or historical report writes."""
import hashlib
...
FOLDER = ROOT / 'private/browser-waveform-beta-v144-test09'
MARKER_TEXT = 'XDJ BETA 09'.encode('utf-16le')
spec = importlib.util.spec_from_file_location('_beta09_primitives', ROOT / 'scripts/build_beta06.py')
...
core.SOURCES = tuple(ROOT / p for p in (
    ...
    'native/waveform_cell.c', 'native/pixel_channels.c', 'native/v144/beta09_hooks.S'))
core.HASH_SOURCES = core.SOURCES + tuple(ROOT / p for p in (
    ...
    'scripts/build_beta09.py', 'scripts/build_beta06.py'))
...
def build(destination=FOLDER):
    ...
    report = {
        'kind': 'test-09-validated-row-waveform-beta', 'numeric_version': '1.44',
        'text_marker': 'XDJ BETA 09', 'text_marker_offsets': list(core.TEXT_OFFSETS),
...
if __name__ == '__main__':
    report = build()
    (ROOT / 'evidence/browser-waveform-beta-v144-test09.json').write_text(json.dumps(report, indent=2) + '\n')
